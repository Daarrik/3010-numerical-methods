...
public class JacobiSeidel {
	...
	// Fills the matrix with values depending on the input chosen by the user in method promptInputMethod(double[][] matrix).
	...
	// Ensures the user inputs a valid coefficient value, recalls itself if otherwise.
	...
	// First checks if the file entered exists, and if it does exist, start reading the coefficients. Ends if an invalid value is encountered. Returns null if the array is not filled.
	public static void readCoefficients(double[][] matrix) {
		...
		try {
			...
			System.out.println("Invalid file or invalid value detected. Check the file, coefficients and/or the number of equations to solve and try again.");
			System.exit(0);
		}
		if(readMatrixFile.hasNextDouble()) {
			System.out.println("There are extra values in the entered file. Check the file or the number of equations to solve.");
			System.exit(0);
		} else {
			System.out.println("Coefficients successfully entered.\n");
		}
